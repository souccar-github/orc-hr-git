...
namespace HRIS.SDKs.Domain.AttendanceSystem.BioMetricDevice.SupportedDevices
{
    ...
    public class BioStationV14 : IBioMetricDevice
    {
        ...
        public List<BioMetricRecordData> GetRecordsData()
        {
            ...
            IntPtr logRecord = Marshal.AllocHGlobal(GetRecordsCount() * Marshal.SizeOf(typeof(BSSDK.BSLogRecord)));
            ...
            do
            {
                ...
                if (result != BSSDK.BS_SUCCESS)
                {
                    ...
                    throw new Exception("GetRecordsData Failed");
                }
            ...
            {
                var record = (BSSDK.BSLogRecord)Marshal.PtrToStructure(new IntPtr(logRecord.ToInt32() + i * Marshal.SizeOf(typeof(BSSDK.BSLogRecord))),
                    typeof(BSSDK.BSLogRecord));

                if (record.userID != 0 && record.tnaEvent <= 3)
                {
                    logCount++;
                    ...
                    {
                        DateTime = eventTime,
                        UserDeviceId = record.userID,
                        RecordType = GetBioMetricRecordType(record.tnaEvent)
                    });
                }
            }
            Marshal.FreeHGlobal(logRecord);
        ...
        public int GetRecordsCount()
        {
            ...
            var result = BSSDK.BS_GetLogCount(Handle, ref numberOfRecords);

            if (result != BSSDK.BS_SUCCESS)
            {
                throw new Exception("GetRecordsCount Failed");
            }
        ...
        public BioMetricRecordType GetBioMetricRecordType(ushort bioMetricRecordType)
        {
            switch (bioMetricRecordType)
            {
                ...
                case 2:
                    return BioMetricRecordType.InMission;

                case 3:
                    return BioMetricRecordType.OutMission;
        ...
        {
            var deletedCount = 0;
            var result = BSSDK.BS_DeleteLog(Handle, 0, ref deletedCount);

            if (result != BSSDK.BS_SUCCESS)
            {
                throw new Exception("ClearRecordsData Failed");
            }
        }

        public List<BioMetricRecordData> GetTestingRecordsData()
        {
            var dirPath = Assembly.GetExecutingAssembly().Location;
            dirPath = Path.GetDirectoryName(dirPath);
            var path = Path.GetFullPath(Path.Combine(dirPath, "\\fingerPrints\\records.json"));
    ...
    internal class BSSDK
    {
        //
        // API Declarations
        //
        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetDeviceID")]
        ...
        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_OpenInternalUDP")]
        public static extern int BS_OpenInternalUDP(ref int handle);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_SearchDeviceInLAN")]
        public static extern int BS_SearchDeviceInLAN(int handle, ref int numOfDevice, ref uint[] deviceIDs,
            ref int[] deviceTypes, ref uint[] readerAddrs);
        ...
        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_ReadConfigUDP")]
        public static extern int BS_ReadConfigUDP(int handle, uint targetAddr, uint targetID, int configType,
            ref int configSize, IntPtr data);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_WriteConfigUDP")]
        public static extern int BS_WriteConfigUDP(int handle, uint targetAddr, uint targetID, int configType,
            int configSize, IntPtr data);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_OpenSocket")]
        public static extern int BS_OpenSocket(string addr, int port, ref int handle);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_CloseSocket")]
        public static extern int BS_CloseSocket(int handle);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetTime")]
        public static extern int BS_GetTime(int handle, ref int timeVal);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_SetTime")]
        public static extern int BS_SetTime(int handle, int timeVal);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetUserDBInfo")]
        public static extern int BS_GetUserDBInfo(int handle, ref int numOfUser, ref int numOfTemplate);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetAllUserInfoBEPlus")]
        public static extern int BS_GetAllUserInfoBEPlus(int handle, IntPtr userHdr, ref int numOfUser);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetAllUserInfoEx")]
        public static extern int BS_GetAllUserInfoEx(int handle, IntPtr userHdr, ref int numOfUser);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetUserInfoBEPlus")]
        public static extern int BS_GetUserInfoBEPlus(int handle, uint userID, IntPtr userHdr);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_DeleteUser")]
        public static extern int BS_DeleteUser(int handle, uint userID);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_DeleteAllUser")]
        public static extern int BS_DeleteAllUser(int handle);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetAllAccessGroupEx")]
        public static extern int BS_GetAllAccessGroupEx(int handle, ref int numOfGroup, IntPtr accessGroup);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetUserBEPlus")]
        public static extern int BS_GetUserBEPlus(int handle, uint userID, IntPtr userHdr, byte[] templateData);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetUserEx")]
        public static extern int BS_GetUserEx(int handle, uint userID, IntPtr userHdr, byte[] templateData);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_EnrollUserBEPlus")]
        public static extern int BS_EnrollUserBEPlus(int handle, IntPtr userHdr, byte[] templateData);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_EnrollUserEx")]
        public static extern int BS_EnrollUserEx(int handle, IntPtr userHdr, byte[] templateData);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_GetLogCount")]
        public static extern int BS_GetLogCount(int handle, ref int logCount);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_ReadLog")]
        public static extern int BS_ReadLog(int handle, int startTime, int endTime, ref int logCount, IntPtr logRecord);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_ReadNextLog")]
        public static extern int BS_ReadNextLog(int handle, int startTime, int endTime, ref int logCount,
            IntPtr logRecord);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_DeleteLog")]
        public static extern int BS_DeleteLog(int handle, int logCount, ref int deletedCount);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_ReadCardIDEx")]
        public static extern int BS_ReadCardIDEx(int handle, ref uint cardID, ref int customID);

        [DllImport("BS_SDK.dll", CharSet = CharSet.Ansi, EntryPoint = "BS_ScanTemplate")]
